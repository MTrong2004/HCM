...
export default function InfographicDuongLoiDenHanhDong() {
  ...
  return (
    <EditorialReveal className="my-0.5 p-3 sm:p-4 rounded-lg bg-paper-light/95 border border-accent/30 shadow-2xs text-ink">
      ...
      {/* Step Indicators */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-2.5" role="tablist">
        {steps.map((st, idx) => {
          ...
          return (
            <button
              ...
              className={`p-2.5 rounded border text-left transition-all min-h-[44px] ${
                isActive
            ...
            >
              <span
                className={`font-mono text-[11px] font-bold block mb-0.5 ${
                  isActive ? "text-accent-light" : "text-accent-dark"
                }`}
              >
                BƯỚC {st.num}
              </span>
              <h5 className="font-serif text-[13px] sm:text-sm font-bold leading-snug">
                {st.title}
              </h5>
            </button>
          );
      ...
      {/* Active Step Panel */}
      <div key={activeStep} className="p-3 sm:p-3.5 rounded bg-paper border border-accent/25 mb-2.5 editorial-tab-panel">
        <span className="text-xs font-mono uppercase text-accent-dark font-bold block mb-1">
      ...
      <div className="p-3 sm:p-3.5 rounded border-l-4 border-accent-dark bg-paper/60 border-t border-r border-b border-accent/20">
        <h6 className="font-serif text-sm sm:text-base font-bold text-primary mb-1">
          Trách nhiệm của Thế hệ Sinh viên, Trí thức trẻ
        </h6>
        <p className="font-sans text-xs sm:text-[13px] text-ink leading-relaxed">
