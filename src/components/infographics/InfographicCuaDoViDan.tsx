  ...
  return (
    <EditorialReveal className="my-0.5 p-3 sm:p-4 rounded-lg bg-paper-light/95 border border-accent/30 shadow-2xs">
        ...
        {pillars.map((pillar, idx) => {
          ...
          return (
            <button
              ...
              className={`p-2 rounded border text-left transition-all duration-200 min-h-[44px] flex flex-col justify-between ${
                isActive
            ...
            >
              <div>
                <span
                  className={`font-mono text-[8.5px] sm:text-[9px] font-bold uppercase tracking-wider block ${
              ...
              <span className="font-sans text-[10px] text-accent-dark font-medium italic truncate">
                {pillar.subtitle}
              </span>
            </button>
          );
      ...
      <div key={activeTab} className="p-2.5 sm:p-3 rounded bg-paper border border-accent/25 editorial-tab-panel">
        ...
        <p className="font-sans text-[11.5px] sm:text-xs text-ink leading-snug mb-2">
          {pillars[activeTab].desc}
        </p>
        <div className="space-y-1 pt-1.5 border-t border-accent/20">
          <span className="text-[10px] font-mono uppercase text-accent-dark font-bold tracking-wider block">
            Luận điểm then chốt:
          </span>
          <ul className="space-y-1 pl-1">
            {pillars[activeTab].details.map((detail, dIdx) => (
              <li
                key={dIdx}
                className="flex items-start gap-1.5 text-[11px] sm:text-[11.5px] text-ink/85 leading-snug"
              >
